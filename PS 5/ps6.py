...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        for i in range(len(abc)):   
            shiftDict[abc[i]] = (abc[shift:] + abc[:shift])[i]
        for i in range(len(ABC)):   
            shiftDict[ABC[i]] = (ABC[shift:] + ABC[:shift])[i]
        return shiftDict
    ...
    def decrypt_message(self):
        ...
        for i in range(1,27):
            ...
            for j in range(len(words)):
                if is_word(self.valid_words, words[j]):
                    count += 1
            trial[i] = count
        best = max(trial.values())
        for i in trial:
            if trial[i] == best:
                s = 26-i      
...
# The file ps6.py contains a helper function get_story_string() that returns the encrypted 
# version of the story as a string. Create a CiphertextMessage object using the story string 
